...
def agglomerative_clustering_defense():
    ...
    print(
        "Percentage of not-poisoned data identified as poisoned: "
        + str(clean_count * 100 // (len(y_poison_dataset) - sum(y_poison_dataset)))
        + "% ("
        + str(clean_count)
        + "/"
        + str(len(y_poison_dataset) - sum(y_poison_dataset))
        + ")"
    )
    print(
        "Percentage of poisoned data identified as poisoned: "
        + str(poisoned_count * 100 // sum(y_poison_dataset))
        + "% ("
        + str(poisoned_count)
        + "/"
        + str(sum(y_poison_dataset))
        + ")"
    )
...
def monitoring_training_dynamics_defense(predictons, number_of_poisoned_samples):
    ...
    for i in range(0, number_of_samples):
        for j in range(0, number_of_samples):
            influence[i][j] = np.sum(
                np.power(
                    (
                        np.array(
                            prediction_changing[i][: len(prediction_changing[0]) - 1]
                        )
                        - np.array(
                            prediction_changing[j][1 : len(prediction_changing[0])]
                        )
                    ),
                    2,
                )
    ...
    print(
        ...
        + str(
            100
            * not_poisoned_identified
            // (number_of_samples - number_of_poisoned_samples)
        )
        + "% ("
        + str(not_poisoned_identified)
        + "/"
        + str(number_of_samples - number_of_poisoned_samples)
        + ")"
    )
    print(
        "Percentage of poisoned data identified as poisoned: "
        + str(100 * poisoned_identified // number_of_poisoned_samples)
        + "% ("
        + str(poisoned_identified)
        + "/"
        + str(number_of_poisoned_samples)
        + ")"
    )
